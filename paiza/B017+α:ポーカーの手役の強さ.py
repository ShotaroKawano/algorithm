...
# 学んだこと
# - isFlashのような変数じゃなくてFlash判定関数を作ることで、コメントで処理の流れを示した後に一行で処理ロジックを書ける
# - 表とかではなく、TDさんのように [1,1,1,1,2], [JOKER,1,1,1,2] と絵に書き出した方がよい
# - ジョーカーを取り除いて考えれば
#   - ジョーカーあってもなくても rank_sizeが2 かつ 個数1のカードがあれば FourCard   [1,1,1,1,2], [JOKER,1,1,1,2]
#   - ジョーカーありでrank_sizeが4 or ジョーカーなしでrank_sizeが5 の場合に 差の絶対値の総和が4以下 ならば Straight   [1,2,3,4,5], [JOKER,2,3,4,5] ??? 合ってる？
# - 総じてTDさん天才 https://github.com/Daisky1113/poker/blob/master/pages/index.vue
# - FiveCardをありでリファクタリングしたい
# - Notionにロジックの考えた形跡を残すのいい
